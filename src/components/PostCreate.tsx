...
export default function PostCreate({ currentUser, onPostCreated }: PostCreateProps) {
  ...
  const handleGenerateAiDraft = async () => {
    if (!aiTopic.trim()) {
      setError('Please enter a topic first for Gemini to draft!');
      return;
    }
    ...
    try {
      ...
      setError(err.message || 'Gemini API was unable to generate. Try again.');
    } finally {
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="flex items-start space-x-3.5">
          ...
          <div className="flex-1 relative">
            <textarea
              ref={textareaRef}
              placeholder={`Share what is on your mind, @${currentUser.username}... Mention friends using @ or tag them!`}
              value={content}
              ...
              <div className="absolute left-0 top-full z-30 mt-2 max-h-48 w-60 overflow-y-auto rounded-xl border border-zinc-200 bg-white p-1.5 shadow-xl dark:border-zinc-800 dark:bg-zinc-950">
                ...
                {filteredAutocompleteUsers.map((u) => (
                  <button
                    ...
                    className="flex w-full items-center space-x-2 rounded-lg px-2.5 py-1.5 text-left text-xs text-gray-700 hover:bg-indigo-50 dark:text-zinc-300 dark:hover:bg-zinc-900/80 transition"
                  >
